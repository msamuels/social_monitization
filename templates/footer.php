<div class="row" id="bottom-nav">
    <div class="col-xs-12"
    ">
    <nav class="navbar-default" id="myNavbar">
        <ul class="nav navbar-nav footer-nav">
            <li><a class="footer-nav" href="/">get started</a></li>
            <li><a class="footer-nav" href="/about-us">about Us</a></li>
            <li><a class="footer-nav" href="/faqs">faq</a></li>
            <li><a class="footer-nav" href="/">avertisements</a></li>
            <li><a class="footer-nav" href="/">blog</a></li>
            <li style="padding-top: 15px"><a href="https://twitter.com/shareitcamp" class="twitter-follow-button"
                                             data-show-count="false">Follow
                    @shareitcamp</a>
                <script async src="//platform.twitter.com/widgets.js" charset="utf-8"></script>
            </li>
            <li style="padding-top: 9px; padding-left: 25px">
                <div class="fb-like" data-href="https://www.facebook.com/shareitcamp" data-layout="standard"
                     data-action="like" data-size="small" data-show-faces="false" data-share="true"></div>
            </li>
        </ul>
    </nav>
</div>
</div>

<div class="row">
    Copyright 2016 shareitcamp.com. All rights reserved<br/>
    Terms of Service | Privacy Policy
</div>

</div>


</body>
